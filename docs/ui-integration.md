...
`33e9f22`에서 API 재시도는 `http_retry.py`로 통합되었다. 검증 브랜치에서 공통 모델의 구문 오류와 search의 상태 연결을 수정했다. runner가 요구하는 memory 이름·상태 계약 오류는 남아 있다. 실제 factory에서 이 오류가 발생하면 상담 연결을 실패 상태로 표시한다. 의료기관 직접 조회는 이 상담 경로와 독립적으로 실행한다.
