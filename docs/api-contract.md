...
- **2026-09-11 실녹화로 발견**: 강원(2023년), 전북(2024년)은 `OO특별자치도`로 행정구역명이
  개칭됐다. `STAGE1="강원도"`로 조회하면 실제 데이터가 있는데도 **0건**이 나온다(강릉·영월로
  확인). `parser.py`의 `SIDO_ALIASES`는 개칭 반영해서 고쳤음 — `geocode`가 카카오에서 옛
  이름을 받아오는 경우가 있다면 조원 3도 이 표를 거쳐가게 해야 한다.
