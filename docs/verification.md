...
**에이전트 상담**은 실제 provider·모델·runner 조립을 시도한다. 공통 모델 구문 오류와 검색 상태 연결을 수정했지만, runner의 메모리 import/상태 계약 오류가 남아 현재 전체 상담 연결은 **BLOCKED**다. 개별 API 인증·기관 조회 성공을 전체 상담의 성공으로 설명하지 않는다.
...
실제 API 검증: 실제 기존 E-Gen client로 반경 5km 내 기관 9곳, 선택 기관의 상세 주소와 대표전화 조회를 확인했다. 카카오는 추가 진단하지 않았다.

CI 실패 수정 범위: 모델 들여쓰기, 검색 세션 상태 초기화, 누락된 검색 테스트 전용 provider, 테스트 fixture의 필수 필드·시간대·주소, 임시 모듈 복원, 연결 오류 재시도, 개인정보 정규식이다. 운영 화면에는 테스트 provider를 연결하지 않는다. 수정 후 로컬 전체 검사는 251 PASS, 실패·수집 오류·skip 0건이었다. 최신 실행 결과는 생성된 보고서와 해당 커밋의 Actions 결과로 확인한다.

## CI 담당 범위 구분

`Web and verification tooling`은 `--scope web`으로 6번 화면·어댑터·검증 도구를 검사한다. `All modules integration`은 기본 전체 검사와 동일하게 `--scope all`을 실행한다. 어느 쪽도 실패를 무시하거나 성공으로 바꾸지 않는다. 전체 검사 통과는 오프라인 테스트 통과이며 실제 API·LLM의 전체 상담 성공을 의미하지 않는다. 각 결과는 `verification-web`, `verification-integration` 아티팩트에 별도로 보존된다. 실패한 pytest 출력도 Actions 단계 로그에 표시한다.

수정 전 원격 실행 `34565779119`는 161 PASS, assertion 실패 1건, 수집 오류 5건이었다. 화면의 SyntaxWarning이 종료 코드 1의 원인이 아니며, 기존 공통 모델·상태 import 오류와 검색 재시도 기대 횟수 차이가 원인이다.
