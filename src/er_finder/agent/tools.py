...
@tool
def list_nearby_ers(lat: float, lon: float, radius_km: int, config: RunnableConfig) -> dict:
...
def get_er_bed_status(
    regions: list[dict[str, str]], hpids: list[str], config: RunnableConfig
) -> dict:
    """목록의 시도·시군구 조합별 병상을 조회·병합하고 목록의 hpid만 반환한다."""
    session = config["configurable"]["session"]
    return session.get_er_bed_status(regions, hpids)


@tool
def get_severe_acceptance(
    regions: list[dict[str, str]], condition: str, config: RunnableConfig
) -> dict:
    """critical/urgent의 조회 조건이 있을 때 시군구별 중증 수용 정보를 조회한다."""
    session = config["configurable"]["session"]
    return session.get_severe_acceptance(regions, condition)
...
def save_visit_plan(hpid: str, name: str, symptom_summary: str, config: RunnableConfig) -> dict:
    ...
    visit = config["configurable"]["visit"]
    return visit.save(hpid, name, symptom_summary)
